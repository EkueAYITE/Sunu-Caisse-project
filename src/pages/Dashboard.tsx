...
const Dashboard: React.FC = () => {
  ...
  const fetchPaiements = async () => {
    try {
      console.log('Début fetchPaiements...');
      // ✅ Correction : Passer les paramètres de pagination
      const response = await apiService.getPaiements(currentPage, 10);
      console.log('Réponse API complète:', response);
      console.log('Type de response:', typeof response);
      console.log('Est-ce un tableau?', Array.isArray(response));

      // ✅ Correction : Suppression de l'accès à .data qui n'existe pas
      const paiementsArray = Array.isArray(response) ? response : [];

      console.log('Tableau final:', paiementsArray);
                ...
                <div className="flex space-x-2">
                  {['espece', 'tpe', 'cheque'].map((mode) => (
                    <button
                      ...
                      onClick={() =>
                        setPaiementForm((prev) => ({ ...prev, mode_paiement: mode, numero_piece: '' }))
                      }
                    >
                      {mode === 'espece'
                        ? 'Espèce'
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    ...
                    <p className="text-xl font-bold text-sunu-gray-dark font-mono">
                      {Array.isArray(filteredPaiements) && filteredPaiements.length > 0
                          ? filteredPaiements.reduce((sum, paiement) => {
                            // ✅ Correction : Conversion explicite en number
                            const montant = typeof paiement.montant === 'number'
                              ? paiement.montant
                              : parseFloat(String(paiement.montant)) || 0;
                            return sum + montant;
                          }, 0).toFixed(0)
                          : '0'} FCFA
                    </p>
                  ...
                  <tr key={paiement.id} className="hover:bg-sunu-gray-light">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm font-medium text-sunu-red font-mono">
                        {/* ✅ Correction : Gestion sécurisée du type number */}
                        {typeof paiement.montant === 'number'
                          ? paiement.montant.toFixed(0)
                          : parseFloat(String(paiement.montant) || '0').toFixed(0)} FCFA
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-sunu-gray-dark font-body">
                      {paiement.mode_paiement === 'espece' ? 'Espèce' :
                        paiement.mode_paiement === 'tpe' ? 'TPE' : 'Chèque'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-sunu-gray-dark font-body">
